...
(function() {
	...
	function RTCMultiSession(connection, callbackForSignalingReady) {
		...
		function newPrivateSocket(_config) {
			...
			var isofferer = _config.isofferer, peer;
			...
			function updateSocket() {
				...
				connection.peers[_config.userid] = {
					...
					onCustomMessage : function(message) {
						log('Received "private" message from', this.userid, isString(message) ? message : toStr(message));
					},
	...
	function PeerConnection() {
		return {
			...
			setRemoteDescription : function(sessionDescription, onSdpSuccess) {
				...
				var self = this;
				this.connection.setRemoteDescription(new RTCSessionDescription(sessionDescription), onSdpSuccess || this.onSdpSuccess, function(error) {
			...
			createDataChannel : function(channelIdentifier) {
				...
				if ((isChrome && this.type == 'offer') || isFirefox) {
					this.setChannelEvents(this.connection.createDataChannel(channelIdentifier || 'channel', dataChannelDict));
				}
			},
	...
	function getUserMedia(options) {
		...
		if (videoMandatoryConstraints) {
			...
			if (mandatory.minWidth && mandatory.minHeight) {
				...
				if (allowed.indexOf(mandatory.minWidth + ':' + mandatory.minHeight) == -1 || allowed.indexOf(mandatory.maxWidth + ':' + mandatory.maxHeight) == -1) {
	...
	var TextSender = {
		send : function(config) {
			...
			var channel = config.channel, _channel = config._channel, initialText = config.text, packetSize = connection.chunkSize || 1000, textToTransfer = '', isobject = false;
	...
	var ReservedExtensionID = 'kfdbepphbiaekljifcldpiapliknffan';
				...
				image.onload = function() {
					...
					setTimeout(function() {
						if (DetectRTC.screen.chromeMediaSource == 'screen') {
							_callback(DetectRTC.screen.chromeMediaSource == 'desktop' ? 'installed-enabled' : 'installed-disabled' /* if chrome extension isn't permitted for current domain, then it will be installed-disabled all the time even if extension is enabled. */
							);
						} else
	...
	function setDefaults(connection) {
		...
		connection.chunkInterval = isFirefox || chromeVersion >= 32 ? 100 : 500;
		// 500ms for RTP and 100ms for SCTP
		connection.chunkSize = isFirefox || chromeVersion >= 32 ? 13 * 1000 : 1000;
		// 1000 chars for RTP and 13000 chars for SCTP

		// www.RTCMultiConnection.org/docs/fakeDataChannels/
		connection.fakeDataChannels = false;

		connection.waitUntilRemoteStreamStartsFlowing = null;
		// NULL == true
		...
		connection.stopMediaStream = function(mediaStream) {
			...
			var checkForMediaStreamTrackStop = Boolean((mediaStream.getAudioTracks || mediaStream.getVideoTracks) && ((mediaStream.getAudioTracks()[0] && !mediaStream.getAudioTracks()[0].stop) || (mediaStream.getVideoTracks()[0] && !mediaStream.getVideoTracks()[0].stop)
			));
